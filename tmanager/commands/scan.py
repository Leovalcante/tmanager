import fnmatch
import os
import sys
import threading
import time

import click
import git

from tmanager.core.file_system import FileSystem
from tmanager.core.messages import Prints, Echoes
from tmanager.core.tool.repository import Repository
from tmanager.utilities import commands as utl_cmd


@click.command(options_metavar="", short_help="Scan filesystem seeking repositories.")
@click.argument("root-dir", required=False, metavar="<root-dir>")
@click.pass_context
def scan(ctx: click.core.Context, root_dir: str) -> None:
    """
    \b
    Scan filesystem seeking repositories starting from <root-dir>.
    If no <root-dir> is specified it will start scanning the system from your home directory.
    \f

    :param click.core.Context ctx: click context
    :param str root_dir: directory to start scanning by searching repositories
    :return: None
    """
    # Get configuration object and verbose
    cfg = utl_cmd.get_configs_from_context(ctx)
    vrb = utl_cmd.get_verbose_from_context(ctx)

    # Set initial variable value
    root_dir = root_dir or FileSystem.get_home_env()

    if not os.path.isdir(root_dir):
        raise click.BadArgumentUsage(Echoes.error(f"{root_dir} is not a directory"))

    Prints.info(f"Using {root_dir} as root dir")
    Prints.info("Scanning system for git repositories. This may take a while")

    # Scan the system for git repositories
    repos_list = []
    Prints.verbose("Start searching daemon", vrb)

    search = threading.Thread(name='repo_search', target=_repo_seeker, args=(root_dir, repos_list))
    search.daemon = True
    search.start()

    try:
        while search.is_alive():
            _animated_loading()
    except KeyboardInterrupt:
        Prints.verbose("Searching deamon has been stopped", vrb)
        Prints.verbose("Quitting...", vrb)
        raise click.Abort()

    # Delete "searching..." line
    sys.stdout.write("\r")

    # Remove already added repositories
    already_added_repo_set = set(map(lambda t: t.get_name(), cfg.get_tools(repo_only=True)))
    repos_list = list(filter(lambda r: r.split("/")[-1] not in already_added_repo_set, repos_list))

    Prints.warning(f"{len(repos_list)} repositories found")
    # Found some repos?
    if repos_list:
        Prints.verbose("Listing repositories", vrb)

        # List repositories found
        i = 0
        for repo in repos_list:
            name = repo.split("/")[-1]
            Prints.info(f"{i + 1}. {name}, {repo}")
            i += 1

        # Ask the user to add all repositories found
        add_it_all = click.confirm(Echoes.input("Do you want to add all of them?"), default=False)

        # OK, add all repositories found to tman
        if add_it_all:
            Prints.info("All repositories found will be added in tman")
            Prints.verbose("All repositories found are going to be added", vrb)
            Prints.verbose("Generating repositories indexes", vrb)

            chosen_indexes = [*range(len(repos_list))]

        # KO, ask to user which repository should be added
        else:
            Prints.verbose("Do not add all repositories", vrb)

            chosen_indexes = click.prompt(Echoes.input("Enter the numbers of the repo you want to add, separated "
                                                       "by a comma"), default="q")

            if chosen_indexes != "q":
                Prints.verbose("There are some repositories to add", vrb)
                Prints.verbose("Validate all user input indexes", vrb)

                # Sanitize input
                chosen_indexes = utl_cmd.sanitize_indexes(repos_list, chosen_indexes)

                Prints.verbose(f"Indexes sanitized: {chosen_indexes}", vrb)

                # If there are any indexes after index sanitize
                if chosen_indexes:
                    Prints.verbose("Listing all repositories user wants to add", vrb)

                    # Print the repositories that will be added

                    if vrb:
                        Prints.warning("The following repositories will be added in tman:")
                        for i in chosen_indexes:
                            path = repos_list[i]
                            name = path.split("/")[-1]
                            Prints.info(f"{name}, {path}")

                else:
                    # There isn't any valid index, print error message and quit
                    Prints.error("None of the supplied indexes is valid")
                    raise click.Abort()

            # User choose to not add any repository
            else:
                Prints.info("No repository will be add")
                return

        Prints.verbose("Start to add desired repositories", vrb)

        # Add selected repositories to tman
        for i in chosen_indexes:
            repo_dir = repos_list[i]
            repo_name = repo_dir.split("/")[-1]
            git_repo = git.Repo(repo_dir)
            try:
                repo_url = git_repo.remote("origin").url
            except ValueError:
                Prints.warning(f"Skipping {repo_name}, no origin found")
                continue

            add_time = time.time()
            repository = Repository(repo_url, repo_dir, name=repo_name, add_date=add_time,
                                    install_date=add_time, last_update_date=add_time)

            Prints.verbose(f"{repository.__str__(True)} is going to be added", vrb)

            if not any(t.get_name() == repository.get_name() for t in cfg.get_tools()):
                cfg.add_tool(repository)
                Prints.success(f"{repository.get_name()} successfully added")

            else:
                Prints.warning(f"{repository.get_name()} has already been added. Skipping...")

        Prints.verbose("All repositories have been added", vrb)
        Prints.verbose("Proceed to save new configurations", vrb)

        cfg.save()

        Prints.verbose("Configurations saved", vrb)
        Prints.verbose("tman scan execution completed", vrb)
    sys.exit(0)


def _repo_seeker(root_dir: str, repo_list: list) -> None:
    """
    Search repositories from root_dir.
    NOTE: THIS FUNCTION IS CALLED WITHIN A THREAD

    :param str root_dir: root directory where to start the scan
    :param list repo_list: repository list to populate
    :return: None
    """
    pattern = ".git"

    for root, dirs, file in os.walk(root_dir):
        for d in dirs:
            if fnmatch.fnmatch(d, pattern):
                repo_list.append(root)


def _animated_loading() -> None:
    """
    Print a loading statement while searching for repositories.

    :return: None
    """
    chars = "/—\\|"
    for char in chars:
        sys.stdout.write("\r" + char + " searching...")
        time.sleep(.1)
        sys.stdout.flush()
